...
for i, (train_index, val_index) in enumerate(kf.split(X_images, binary_labels)):
    ...
    resnet_advanced.fit(X_train, y_train, epochs=100, validation_data=(X_val, y_val), class_weight=class_weights_dict)
    
    y_pred_probs = resnet_advanced.predict(X_val)[:, 1]
...
plt.tight_layout()
plt.savefig("C:/Users/nirma/Documents/paper_3_code/roc_auc_curve_Type_1_ResNet50.png", dpi=300, bbox_inches='tight')
plt.show()
    
y_pred = np.argmax(resnet_advanced.predict(X_val), axis=1)
