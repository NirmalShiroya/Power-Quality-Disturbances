...
# Load and merge data
train_data = pd.read_csv('C:/Users/nirma/Downloads/Cleaned_Data_with_Disturbance_Type_1-6.csv')
test_data = pd.read_csv('C:/Users/nirma/Downloads/Cleaned_Data_with_Disturbance_Type_7-12.csv')
merged_data = pd.concat([train_data, test_data], ignore_index=True)

# Function to extract patterns using only THD-V Max(%)
def extract_patterns(data, disturbance_column, thdv_column=['Upstream Voltage Min(kV)'], window=5):
    patterns = []
    labels = []
    time_points = []
...
X_train, X_test, y_train, y_test = train_test_split(
    X_images,
    binary_labels,
    test_size=0.3,  # Set test size to 30%
    random_state=42,
...
X_train_final, X_val, y_train_final, y_val = train_test_split(
    X_train,
    y_train,
    test_size=0.2,  # 20% of 70% (14% of total data) will be validation
    random_state=42,
    stratify=y_train
)
...
if len(cnn_matched_indices) < 4:
    ...
    for ax, index in zip(axes, selected_indices):
        ...
        ax.set_ylabel('Normalized Upstream Voltage (Index)')  # Use voltage values for y-axis
